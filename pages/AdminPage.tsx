...
const AdminPage: React.FC<AdminPageProps> = ({
  ...
  // Token refresh logic removed — auth token handling is managed elsewhere.
  ...
  useEffect(() => {
    const loadCredentials = async () => {
      try {
        ...
        // Wait briefly for Firebase auth to be ready so we can attach a token
        const waitForAuthUser = async (timeoutMs = 3000) => {
        ...
        const maybeUser = await waitForAuthUser(3000);
        const auth = getFirebaseAuth();
        let token: string | null = null;
        try {
          token = maybeUser ? await auth.currentUser.getIdToken() : null;
        } catch (e) {
          token = null;
        }
  ...
  const handleSaveCredentials = async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body?.error || "Unauthorized: insufficient privileges");
      }
  ...
  const handleSaveFeedbackUrls = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text().catch(() => "");
        throw new Error(errorText || "Failed to save feedback URLs");
      }
    ...
    <>
      {showError && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-95">
          <div className="max-w-md w-full p-8 rounded-2xl shadow-xl border border-red-200 bg-white flex flex-col items-center">
            <svg
            ...
            <p className="text-base text-gray-700 mb-4 text-center">
              {errorMessage || "Unauthorized: insufficient admin privileges"}
            </p>
            <button
              className="px-5 py-2 rounded-lg bg-gray-900 text-white font-semibold shadow hover:bg-gray-800 transition"
      ...
      <div
        className="min-h-screen grid-pattern relative overflow-hidden"
        style={{
          filter: showError ? "blur(2px)" : "none",
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-10">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4">
                <div className="space-y-1 flex-1">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">
                    Manage clients, monitor system activity, and configure
                    Twilio credentials.
                  </p>
                ...
                <div className="flex gap-1.5 sm:gap-2 items-center flex-wrap">
                  ...
                  <button
                    onClick={() => {
                      // Remove local session and logout
                      try {
          ...
          <div className="mb-6 sm:mb-8 lg:mb-10">
            <div className="premium-card bg-white border border-gray-200 rounded-2xl p-4 sm:p-5 shadow-sm">
              {/* User details placeholder, Firebase removed */}
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setLocalMessagingServiceSid(e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-gray-900 bg-white text-gray-900 shadow-sm"
          ...
          {/* Registered Clients - Shows all Firebase Auth login clients with profile styling */}
          <div className="gradient-border glow-on-hover premium-card bg-white shadow-lg p-4 sm:p-5 lg:p-6 xl:p-8 transition-all duration-300 hover:shadow-xl rounded-2xl mb-6 sm:mb-8 lg:mb-10">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                <select
                  ...
                  value={
                    search === "Active" || search === "Suspended"
                      ? search
                      : "All"
                  }
                >
                  ...
                  {users
                    ...
                    .map((user) => {
                      ...
                      const initials = email
                        ...
                        : "?";
                      // Show business name from profile.businessName (stored when user edits in dashboard)
                      // Fallback hierarchy: profile.businessName → company.companyName → email
                      const businessName =
          ...
          {/* Analytics Section - Enhanced UI with Real Data */}
          <div className="gradient-border glow-on-hover premium-card bg-white shadow-lg p-4 sm:p-5 lg:p-6 xl:p-8 transition-all duration-300 hover:shadow-xl rounded-2xl">
            ...
            {/* Customer Growth Chart (30D) - Using Real Data */}
